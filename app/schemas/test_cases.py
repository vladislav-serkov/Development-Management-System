...
class TestCaseAskResult(BaseModel):
    ...
    covered_by: list[str] = Field(
        ...
        description="Если запрошенный пункт уже покрыт — ТОЧНЫЕ названия существующих кейсов, которые его покрывают (копировать из списка существующих кейсов дословно)",
    )
