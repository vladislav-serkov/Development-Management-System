...
DETECT_FEATURE_PROMPT = (
    ...
    "has_detailed_mapping (true если шаг содержит структурированный маппинг полей — см. правило ниже), "
    ...
    "или 'Добавить запись product_schedule_date для каждой даты из $.dates[]'), "
    "БЕЗ перечисления полей/элементов маппинга — детальный маппинг будет извлечён отдельно\n"
    "- Установи has_detailed_mapping: true если шаг содержит ЛЮБОЕ из:\n"
    "  * Таблицу маппинга XML/JSON полей/элементов сообщения\n"
    ...
    "  Общий признак: шаг содержит 3+ пар 'поле = значение/источник'"
)
...
def build_mapping_prompt(feature_name: str, feature_type: str, steps_list: str) -> str:
    ...
    return (
        ...
        "это может быть таблица маппинга XML/JSON сообщения, перечисление полей для INSERT/UPDATE в БД-таблицу, "
